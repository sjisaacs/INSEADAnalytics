...
getwd()
setwd("Exercises/Exerciseset2/SI_ExerciseSet2/")
list.files()
```
...
2.    
`r row.names(Rotated_Factors)[1]`: `r Rotated_Factors[1,1]`,    
`r row.names(Rotated_Factors)[2]`: `r Rotated_Factors[2,1]`,    
`r row.names(Rotated_Factors)[3]`: `r Rotated_Factors[3,1]`,    
...
```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
#filter data according to date ranges
futures_dataPC <- futures_data[row.names(futures_data)>"2002-11-01"&row.names(futures_data)<"2007-10-01",]
futures_dataCrisis <- futures_data[row.names(futures_data)>"2007-10-01"&row.names(futures_data)<"2009-03-01",]

#perform PCA on new pre-crisis data set
Variance_Explained_Table_resultsPC<-PCA(futures_dataPC, graph=FALSE)
Variance_Explained_TablePC<-cbind(paste("component",1:ncol(futures_dataPC),sep=" "),Variance_Explained_Table_resultsPC$eig)
Variance_Explained_TablePC<-as.data.frame(Variance_Explained_TablePC)
colnames(Variance_Explained_TablePC)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")

#perform PCA on new crisis data set
Variance_Explained_Table_resultsCrisis<-PCA(futures_dataCrisis, graph=FALSE)
Variance_Explained_TableCrisis<-cbind(paste("component",1:ncol(futures_dataCrisis),sep=" "),Variance_Explained_Table_resultsCrisis$eig)
Variance_Explained_TableCrisis<-as.data.frame(Variance_Explained_TableCrisis)
colnames(Variance_Explained_TableCrisis)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")
```


```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
#plot chart of pre-crisis PCA results
show_dataPC = data.frame(Variance_Explained_TablePC)
m1PC<-gvisTable(show_dataPC,options=list(width=1920, height=min(400,27*(nrow(show_dataPC)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1PC,'chart')
```
...
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
#plot chart of crisis PCA results
show_dataCrisis = data.frame(Variance_Explained_TableCrisis)
m1Crisis<-gvisTable(show_dataCrisis,options=list(width=1920, height=min(400,27*(nrow(show_dataCrisis)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1Crisis,'chart')
```

<br>

```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
eigenvaluesPC  <- Variance_Explained_TablePC[,2]
```

```{r Fig2, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
#pre-crisis scree plot
dfPC           <- cbind(as.data.frame(eigenvaluesPC), c(1:length(eigenvaluesPC)), rep(1, length(eigenvaluesPC)))
colnames(dfPC) <- c("eigenvalues", "components", "abline")
LinePC         <- gvisLineChart(as.data.frame(dfPC), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Pre-Crisis Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(LinePC, 'chart')
```
...
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
eigenvaluesCrisis  <- Variance_Explained_TableCrisis[,2]
```

```{r Fig3, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
#crisis scree plot
dfCrisis           <- cbind(as.data.frame(eigenvaluesCrisis), c(1:length(eigenvaluesCrisis)), rep(1, length(eigenvaluesCrisis)))
colnames(dfCrisis) <- c("eigenvalues", "components", "abline")
LineCrisis         <- gvisLineChart(as.data.frame(dfCrisis), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Crisis Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(LineCrisis, 'chart')
```

5. Assets are more highly correlated during bear markets than bull markets. 50% of the variability in the crisis data can be explained by fewer factors (`r substr(show_dataCrisis[show_dataCrisis[4]>50][1],nchar(show_dataCrisis[show_dataCrisis[4]>50][1]),nchar(show_dataCrisis[show_dataCrisis[4]>50][1]))`) than 50% of variability of the pre-crisis data (`r substr(show_dataPC[show_dataPC[4]>50][1],nchar(show_dataPC[show_dataPC[4]>50][1]),nchar(show_dataPC[show_dataPC[4]>50][1]))`).
...
1. The code above does the following:   
* iterates through all the moving averages in MAfreq (`r paste(MAfreq, collapse=", ")`) calculating the moving average of the past n days of price data for each window and each future, storing these values in matrix signal_used
* the code then normalises each row of matrix signal_used so that the total of each row is equal to 1 (if not 0)
* we then create the trade signal by shifting our MA 'vote' signal by two days, applying it to the futures data and summing across all futures signals for each day (i.e. sum across all rows)


2. Below is a similar trading strategy using shorter MA signals:

```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used_new <- 0*futures_data
MAfreq_new <- seq(3,10,by=1)
for (iter in 1:length(MAfreq_new))
  signal_used_new = signal_used_new + sign(apply(futures_data, 2, function(r) ma(r,MAfreq_new[iter])))
signal_used_new = t(apply(signal_used_new,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
    res = r/sum(abs(r))
  res
}))
colnames(signal_used_new) <- colnames(futures_data)
strategy_by_future_new <- scrub(shift(signal_used_new,1)*futures_data) #signal leads by only 1 day rather than 2
trading_strategy_new = apply(strategy_by_future_new,1,sum)
names(trading_strategy_new) <- rownames(futures_data)

#Seeing how this new strategy performs
pnl_plot(trading_strategy_new)
```

The returns of this strategy are considerably worse than the example given in the exercise (returns of 1.8 vs 6.2, lower Sharpe, larger drawdowns).

Below is a third MA strategy which attempts to get better returns:

```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
futures_data_ag <- futures_data[,1:13]
futures_data_energy <- futures_data[,14:20]
futures_data_metal <- futures_data[,21:29]
futures_data_fi <- futures_data[,30:43]
futures_data_index <- futures_data[,44:55]
futures_data_fx <- futures_data[,56:64]

futures_data_comb <- cbind(futures_data_energy, futures_data_metal, futures_data_ag)

signal_used_3 <- 0*futures_data_comb
MAfreq_3 <- seq(15,150,by=30)
for (iter in 1:length(MAfreq_3))
  signal_used_3 = signal_used_3 + sign(apply(futures_data_comb, 2, function(r) ma(r,MAfreq_3[iter])))
signal_used_3 = t(apply(signal_used_3,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
    res = r/sum(abs(r))
  res
}))
colnames(signal_used_3) <- colnames(futures_data_comb)
strategy_by_future_3 <- scrub(shift(signal_used_3,1)*futures_data_comb)
trading_strategy_3 = apply(strategy_by_future_3,1,sum)
names(trading_strategy_3) <- rownames(futures_data_comb)

#Seeing how this new strategy performs
pnl_plot(trading_strategy_3)
```

This strategy uses only ag, energy and metal futures to generate its signal, has a similar (slightly shorter) set of MA signals, and lags the signal by only 1 day.
...
**Finally**: One can develop (shiny based) interactive versions of this report and deploy them using `shinyapps::deployApp('SI_ExerciseSet2.Rmd')` (you need a [shinyapps.io](https://www.shinyapps.io) account for this). This is for example an  [interactive version of this exercise.](https://inseaddataanalytics.shinyapps.io/ExerciseSet2/)
