...
AAPL Cumulative Returns: `r round(100*sum(StockReturns[,2]),1)` = 596.4%
AAPL Average Daily Returns: `r round(100*mean(StockReturns[,2]),3)` = 0.148%
AAPL Standard Deviation: `r round(100*sd(StockReturns[,2]),3)` = 2.39%
...
**Your Answers here:**
Yahoo! ticker symbol: YHOO

Add YHOO as 3rd element of 'mytickers' vector, and therefore 3rd column of 'StockPrices' matrix. Then,

YHOO Cumulative Returns: `r round(100*sum(StockReturns[,3]),1)` = 289%
YHOO Average Daily Returns: `r round(100*mean(StockReturns[,3]),3)` = 0.072%
YHOO Standard Deviation: `r round(100*sd(StockReturns[,3]),3)` = 3.02%
...
number of rows: nrow(transposedData) = 3 (since I added YHOO to 'mytickers' in dataSet1.R); otherwise 2
number of columns: ncol(transposedData) = 4029 
...
In dataSet1.R line 9, startDate is already equal to "2001-01-01". From `r head(StockReturns)` we see that the data actually starts on 2001-01-03, since there is no SPY data for 2001-01-01 and 2001-01-02 (these were not trading days).
...
Therefore, each row of myData is different from each row of StockReturns - it is actually a concatenation of all columns of StockReturns. This makes `r myData != StockReturns` true (i.e. 1) for all rows in myData (of which there are 20) and in turn makes the sum = 20.
